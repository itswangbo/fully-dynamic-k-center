		    ...
		    unsigned int set_index)
{
	...
	set->elm_ptr = (struct element_pointer *)malloc_wrapper(sizeof(*(set->elm_ptr)) * range);
	memset(set->elm_ptr, -1, sizeof(*(set->elm_ptr)) * range);
	//set->elements = (unsigned int *)malloc_wrapper(sizeof(unsigned int) * max_size);
			     ...
			     unsigned int range)
{
	...
	for (i = 1; i < n; i++) {
		...
		//sets[i].elements = (unsigned int *)malloc_wrapper(sizeof(unsigned int) * max_size);
...
void remove_element_set(struct set * set, unsigned int element)
{
	...
	assert(set->index == set->elm_ptr[element].set_index);
	assert(0 < set->card);
	...
	set->elements.at(position) = set->elements.at(set->card);
