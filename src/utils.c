...
{
  //in linux, use the next line to get current usage of memory
...
long memory_usage() {
	...
	while(fgets(buf, sizeof(buf)-1, fp) != NULL){
		if(sscanf(buf, "VmPeak:%lu", &vm_peak) > 0) break;
	}
	fclose(fp);
	return vm_peak;
/*
	struct statm_t *usage =  (struct statm_t *)malloc_wrapper(sizeof(statm_t));
	read_off_memory_status(usage);
	return (long)usage->size;
*/
}
