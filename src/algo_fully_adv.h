...
typedef struct {
	...
	std::vector<unsigned int> selected_centers;
	unsigned int covered_points;
	double true_radius;
	std::unordered_map<unsigned int, std::vector<unsigned int> > selected_clusters;
} Fully_adv_cluster;
...
void fully_adv_k_center_run(Fully_adv_cluster levels[],
			    ...
			    unsigned int *helper_array, unsigned int k);
#endif
