						   ...
						   unsigned int k)
{
	...
	tmp = (struct lookup_node *)calloc_wrapper(1,sizeof(*tmp));
	tmp->element = element;
	tmp->lookup_list = (struct interval *)calloc_wrapper(k, sizeof(*tmp->lookup_list));
	return tmp;
}
/**
...
{
	lookup->leftovers =
	    (struct leftovers *)calloc_wrapper(lookup->nb_level + 1, sizeof(*lookup->leftovers));
	lookup->leftovers_ptr =
	    (struct leftovers_ptr *)calloc_wrapper(lookup->range_elements,
...
{
	lookup->lookup_table =
		(struct lookup_node **)calloc_wrapper( k * nb_level, sizeof(*lookup->lookup_table));
	lookup->tmp = (struct interval *)malloc_wrapper(sizeof(*lookup->tmp) * k);
	lookup->k = k;
	lookup->nb_level = nb_level;
	lookup->range_elements = range_elements;
	lookup->elements =
		(struct lookup_info *)calloc_wrapper(lookup->range_elements,sizeof(*lookup->elements));
...
void create_leaf_element_lookup(struct lookup_table * lookup, unsigned int element)
{
	...
	info->leaf = tmp;
	info->clusters=(struct interval *)calloc_wrapper(lookup->k,sizeof(*info->clusters));
}
 ...
 */
static void split_lookup_list(struct interval *_old, struct interval *_new, unsigned int length, unsigned int level, unsigned int cluster)
{
	if (cluster + 1 < length)
		memset(_new + cluster + 1, 0,
		       (length - cluster - 1) * sizeof(*_new));
	if (cluster) {
		memcpy(_new, _old, cluster * sizeof(*_old));
		memset(_old, 0, cluster * sizeof(*_new));
	}
	_new[cluster].end = _old[cluster].end;
	_new[cluster].begin = level;
	_old[cluster].end = level;
}
...
static void increase_leftovers_capacity(struct leftovers *leftovers)
{
	...
	if (size == 0) {
		leftovers->elements =
		    (unsigned int *)malloc_wrapper(sizeof(*leftovers->elements) *
				   INIT_LEFTOVERS_SIZE);
		size = INIT_LEFTOVERS_SIZE;
	} else {
		leftovers->elements =
		    (unsigned int *)realloc_wrapper(leftovers->elements, &size,
